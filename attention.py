# ------------------------------------------
# Copyright (c) 2015-present, Facebook, Inc.
# All rights reserved.
# ------------------------------------------
# Modification:
# Added code for dualprompt implementation
# -- Jaeho Lee, dlwogh9344@khu.ac.kr
# ------------------------------------------
import torch
import torch.nn as nn
import torch.nn.functional as F

from subnet import SubnetLinear, SubnetLinearQKV, SubnetLinearV1, SubnetLinearQKVV1

from loralib.layers import LinearCL, MergedLinearCL

class PreT_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., sparsity=0.0):
        super().__init__()
        assert dim % num_heads == 0, 'dim should be divisible by num_heads'
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5

        self.sparsity = sparsity
        self.subnet = True

        # weight [2304, 768]
        # bias [2304]
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
        self.attn_map = None

    def forward(self, x, prompt, prompt_idx=None):

        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)

        # Wq * x_t Collect
        self.key_ = q

        if prompt is not None:
            # prefix key, value
            prompt = prompt.permute(1, 0, 3, 2, 4).contiguous() # 2, B, num_heads, prompt_length, C // num_heads
            key_prefix = prompt[0] # B, num_heads, prompt_length, embed_dim // num_heads
            value_prefix = prompt[1] # B, num_heads, prompt_length, embed_dim // num_heads

            expected_shape = (B, self.num_heads, C // self.num_heads)
            
            assert (key_prefix.shape[0], key_prefix.shape[1], key_prefix.shape[3]) == expected_shape, f'key_prefix.shape: {key_prefix.shape} not match k.shape: {k.shape}'
            assert (value_prefix.shape[0], value_prefix.shape[1], value_prefix.shape[3]) == expected_shape, f'value_prefix.shape: {value_prefix.shape} not match v.shape: {v.shape}'

            k = torch.cat([key_prefix, k], dim=2)
            v = torch.cat([value_prefix, v], dim=2)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        
        if prompt is not None:
            self.attn_map = attn

        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        
        x = self.proj(x)
        x = self.proj_drop(x)
        return x


class Soft_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., sparsity=0.0):
        super().__init__()
        assert dim % num_heads == 0, 'dim should be divisible by num_heads'
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5
        self.sparsity = sparsity

        # weight [2304, 768]
        # bias [2304]
        self.qkv = SubnetLinearQKVV1(dim, dim * 3, bias=qkv_bias, sparsity=self.sparsity)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = SubnetLinearV1(dim, dim, bias=True, sparsity=self.sparsity)
        self.proj_drop = nn.Dropout(proj_drop)

        self.attn_map = None

    def forward(self, x, prompt, prompt_idx=None):

        B, N, C = x.shape
        mode = 'train' if self.training else 'valid'
        subnet_id = prompt_idx.item() if prompt_idx is not None else 0
        qkv = self.qkv(x, mode=mode, subnet_id=subnet_id).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)

        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)

        # Wq * x_t Collect
        self.key_ = q

        if prompt is not None:
            # prefix key, value
            prompt = prompt.permute(1, 0, 3, 2, 4).contiguous() # 2, B, num_heads, prompt_length, C // num_heads
            key_prefix = prompt[0] # B, num_heads, prompt_length, embed_dim // num_heads
            value_prefix = prompt[1] # B, num_heads, prompt_length, embed_dim // num_heads

            expected_shape = (B, self.num_heads, C // self.num_heads)
            
            assert (key_prefix.shape[0], key_prefix.shape[1], key_prefix.shape[3]) == expected_shape, f'key_prefix.shape: {key_prefix.shape} not match k.shape: {k.shape}'
            assert (value_prefix.shape[0], value_prefix.shape[1], value_prefix.shape[3]) == expected_shape, f'value_prefix.shape: {value_prefix.shape} not match v.shape: {v.shape}'

            k = torch.cat([key_prefix, k], dim=2)
            v = torch.cat([value_prefix, v], dim=2)

        attn = (q @ k.transpose(-2, -1)) * self.scale

        if prompt is not None:
            self.attn_map = attn

        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)

        # if prompt_idx is not None:
        mode = 'train' if self.training else 'valid'
        subnet_id = prompt_idx.item() if prompt_idx is not None else 0
        x = self.proj(x, mode=mode, subnet_id=subnet_id)

        x = self.proj_drop(x)
        return x
    


class LoRA_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., sparsity=0.0, param_ratio=4):
        super().__init__()
        assert dim % num_heads == 0, 'dim should be divisible by num_heads'
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5

        self.sparsity = sparsity
        self.subnet = True

        # lora hyper-params
        nx = dim # 1024
        n_state = dim # 1024
        #param_ratio = 4 #125 # 6.9M params
        lora_attn_dim = 4 * param_ratio # config
        lora_attn_alpha = 32 * param_ratio # config
        lora_dropout = 0.1
        self.attn_map = None

        # weight = [2304, 786]
        # bias = [2304]

        #if self.sparsity <= 0.0:
        #    self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        #else:
        self.qkv = MergedLinearCL(
            nx, n_state * 3, 
            r=lora_attn_dim, 
            lora_alpha=lora_attn_alpha, 
            lora_dropout=lora_dropout, 
            enable_lora=[True, False, True], 
            fan_in_fan_out=False,
            merge_weights=False
            )

        self.attn_drop = nn.Dropout(attn_drop)

        #if self.sparsity <= 0.0 :
        #    self.proj = nn.Linear(dim, dim)
        #else:
        self.proj = LinearCL(
            nx, n_state * 1, 
            r=lora_attn_dim, 
            lora_alpha=lora_attn_alpha, 
            lora_dropout=lora_dropout, 
            fan_in_fan_out=False,
            merge_weights=False
            )

        self.proj_drop = nn.Dropout(proj_drop)

    def forward(self, x, prompt, prompt_idx=None):

        B, N, C = x.shape
        #if self.sparsity <= 0.0:
        #    qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        #else:
        mode = 'train' if self.training else 'valid'
        subnet_id = prompt_idx.item() if prompt_idx is not None else 0
        qkv = self.qkv(x, mode=mode, subnet_id=subnet_id).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) 

        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)

        # Wq * x_t Collect
        self.key_ = q

        if prompt is not None:
            # prefix key, value
            prompt = prompt.permute(1, 0, 3, 2, 4).contiguous() # 2, B, num_heads, prompt_length, C // num_heads
            key_prefix = prompt[0] # B, num_heads, prompt_length, embed_dim // num_heads
            value_prefix = prompt[1] # B, num_heads, prompt_length, embed_dim // num_heads

            expected_shape = (B, self.num_heads, C // self.num_heads)
            
            assert (key_prefix.shape[0], key_prefix.shape[1], key_prefix.shape[3]) == expected_shape, f'key_prefix.shape: {key_prefix.shape} not match k.shape: {k.shape}'
            assert (value_prefix.shape[0], value_prefix.shape[1], value_prefix.shape[3]) == expected_shape, f'value_prefix.shape: {value_prefix.shape} not match v.shape: {v.shape}'

            k = torch.cat([key_prefix, k], dim=2)
            v = torch.cat([value_prefix, v], dim=2)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        if prompt is not None:
            self.attn_map = attn

        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)

        #if self.sparsity <= 0.0 :
        #    x = self.proj(x)
        #else:
        # if prompt_idx is not None:
        mode = 'train' if self.training else 'valid'
        subnet_id = prompt_idx.item() if prompt_idx is not None else 0
        x = self.proj(x, mode=mode, subnet_id=subnet_id)

        x = self.proj_drop(x)
        return x

