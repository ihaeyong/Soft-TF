...
from register import ERegister
from attention import PreT_Attention, LoRA_Attention, Soft_Attention

from subnet import SubnetMlp, AdaptMlp
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., sparsity=0.01, param_ratio=4, subnet=None):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn_layer=Attention, mlp_layer=Mlp,
            sparsity=0.0, param_ratio=4, subnet=None):
        super().__init__()

        self.sparsity = sparsity
        self.norm1 = norm_layer(dim)
        if subnet == 'lora':
            self.attn = attn_layer(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, 
                                proj_drop=drop, sparsity=sparsity, param_ratio=param_ratio)
        else:
            self.attn = attn_layer(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, 
                                proj_drop=drop, sparsity=sparsity)
        ...
        self.subnet = subnet
        self.mlp_cls = mlp_layer.__name__

        if self.subnet == 'adapter' and mlp_layer.__name__ == 'AdaptMlp':
            self.mlp = mlp_layer(in_features=dim, hidden_features=int(dim * mlp_ratio), 
                                act_layer=act_layer, drop=drop, param_ratio=param_ratio)
        elif self.subnet == 'soft' and mlp_layer.__name__ == 'SubnetMlp':
            self.mlp = mlp_layer(in_features=dim, hidden_features=int(dim * mlp_ratio), 
                                 act_layer=act_layer, drop=drop, sparsity=sparsity)
        else:
            self.mlp = mlp_layer(in_features=dim, hidden_features=int(dim * mlp_ratio), 
    ...
    def forward(self, x, prompt=None, prompt_idx=None):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), prompt, prompt_idx)))

        if self.subnet == 'adapter' and self.mlp_cls == 'AdaptMlp':
            x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x), prompt_idx=prompt_idx)))
        elif self.subnet == 'soft' and self.mlp_cls == 'SubnetMlp':
            x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x), prompt_idx)))
        else:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            sparse_patch_c=0.0, num_register_tokens=0, sparsity=0.0, use_lora=False, target_layers=None, subnet=None, param_ratio=None):
        """
        Args:
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            ...
            #self.e_register = ERegister(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
            #        prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size=pool_size, top_k=top_k, batchwise_prompt=batchwise_prompt,
            #        prompt_key_init=prompt_key_init, num_layers=num_e_prompt, use_prefix_tune_for_e_prompt=use_prefix_tune_for_e_prompt,
            #        num_heads=num_register_tokens, same_key_value=same_key_value, 
            #        sparse_patch_c=sparse_patch_c, img_size=img_size, patch_size=patch_size) 
        ...
        for i in range(depth):
            if self.subnet in ['soft', 'lora', 'adapter']: 
        ...
        for i in range(depth):
            if i in self.target_layers:
                if self.subnet == 'soft':
                    attn_layer = Soft_Attention
                    mlp_layer = SubnetMlp
                elif self.subnet == 'lora':
                    attn_layer = LoRA_Attention
                    mlp_layer = Mlp
                elif self.subnet == 'adapter':
                    attn_layer = PreT_Attention
                    mlp_layer = AdaptMlp
                else:
                    pass
            ...
            block = block_fn(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, 
                            init_values=init_values, drop=drop_rate, attn_drop=attn_drop_rate, 
                            drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,
                            attn_layer=attn_layer, mlp_layer=mlp_layer, sparsity=sparsity_list[i], 
                            subnet=subnet, param_ratio=param_ratio)
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False, mode='train', infer_id=-1, gt_id=-1):

        batch, device = x.shape[0], x.device
        x = self.patch_embed(x) # 24, 196, 768
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                if self.use_e_prompt:
                    # res, x = self.e_register(x, prompt_mask=prompt_mask, cls_features=cls_features, task_id=task_id)
                    # self.register_tokens = res['batched_register']
                    ...
                    res = self.e_prompt(x, prompt_mask=prompt_mask, cls_features=cls_features, infer_id=infer_id, gt_id=gt_id, mode=mode)
                    e_prompt = res['batched_prompt']
                    e_prompt_idx = res['prompt_idx']
                else:
                    res = dict()
                ...
                if mode == 'infer':
                    if infer_id == -1:
                        t_id = torch.ones_like(e_prompt_idx[0]) * -2
                    elif infer_id > 0:
                        t_id = torch.ones_like(e_prompt_idx[0]) * infer_id
                else:
                ...
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx:
                        if self.use_prefix_tune_for_g_prompt:
                            ...
                            g_prompt=None
                        x = block(x, prompt=g_prompt, prompt_idx=torch.ones_like(e_prompt_idx[0]) * -1)
                        self.g_prefix_feature[i] = {"key": block.attn.key_}
                        #self.attn_map[i] = {"attn": block.attn.attn_map}
                        ...
                        if self.use_prefix_tune_for_e_prompt:
                            ...
                            x = block(x, prompt=e_prompt[e_prompt_counter], prompt_idx=t_id)
                        ...
                        if self.sparsity > 0.0 and False:
                            x = block(x, prompt_idx=t_id)
                        else:
                            x = block(x)
            else:
                x = self.blocks(x)
    ...
    def forward(self, x, task_id=-1, cls_features=None, train=False, mode='train', infer_id=-1, gt_id=-1):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train, mode=mode, infer_id=infer_id, gt_id=gt_id)
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    for i, block in enumerate(model.blocks.children()):
        ...
        for r in range(2):
            #if hasattr(getattr(block.mlp, f'fc{r + 1}'), 'weight'):
